// build-geo-posts-brandpulse-ai.mjs
// Reads GEO article markdown files from opc-doc (brandpulse-ai), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-brandpulse-ai'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/brandpulse-ai/data/geoPosts.ts'
// NOTE: Vercel alias is NOT brandpulse-ai.vercel.app — after first deploy, replace both with the
// real alias (e.g. brandpulse-ai-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://brandpulse-ai.lxsaihub.com'
const LIVE_DOMAIN = 'https://brandpulse-ai.lxsaihub.com'
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try BrandPulse</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/brandpulse-ai\.vercel\.app/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/brandpulse-ai\.vercel\.app/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    ...
    description,
    html: mdToHtml(body).replace(/https:\/\/brandpulse-ai\.vercel\.app/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-brandpulse-ai.mjs — do not edit by hand.
